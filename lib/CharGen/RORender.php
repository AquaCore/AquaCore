...
abstract class RORender
{
	...
	final protected function renderImage( &$img, $param )
	{
		...
		if (
			get_class($this) === 'CharGen\CharacterHeadRender' ||       // Only head
			(
				get_class($this) === 'CharGen\CharacterRender' &&           // Only for character
		...
		else
		{
			...
			if( !empty($param['body']) ) {
				...
				if( get_class($this) === 'CharGen\CharacterRender' &&
